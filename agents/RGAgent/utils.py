...
def _calculate_request_cost(
    ...
    # Combine output + reasoning for billing (reasoning is part of generated tokens)
    ...
    if normalized_model in ("google/gemini-2.5-pro", "google/gemini-3-pro-preview"):
        ...
        if billed_output_tokens <= threshold:
            cost += billed_output_tokens * prices["output_low"] / 1_000_000
        else:
            cost += threshold * prices["output_low"] / 1_000_000
            cost += (billed_output_tokens - threshold) * prices["output_high"] / 1_000_000
    ...
    # Handle flat pricing for other models
        ...
        if "output" in prices:
            cost += billed_output_tokens * prices["output"] / 1_000_000
    ...
    async def generate() -> ModelOutput:
        ...
        if output.usage:
            ...
            try:
                ...
                # Log the cost
                reasoning_tokens = output.usage.reasoning_tokens or 0
                cached_tokens = output.usage.input_tokens_cache_read or 0
                print(
                    f"Request cost: ${cost:.6f} for {str(self)} "
                    f"({output.usage.input_tokens} in, {cached_tokens} cached, "
                    f"{output.usage.output_tokens} out, {reasoning_tokens} reasoning)"
                )
                logger.info(
                    f"Request cost: ${cost:.6f} for {str(self)} "
                    f"({output.usage.input_tokens} in, {cached_tokens} cached, "
                ...
                # Log session total
                print(f"Session total cost: ${session_total:.6f} (limit: ${budget_limit:.2f})" if budget_limit > 0 else f"💳 Session total cost: ${session_total:.6f}")
