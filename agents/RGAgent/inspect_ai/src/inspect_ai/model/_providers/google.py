...
class GoogleGenAIAPI(ModelAPI):
    ...
    def should_retry(self, ex: Exception) -> bool:
        ...
        def status_code(err: Exception) -> int | None:
            status = (
                getattr(err, "status", None)
                or getattr(err, "code", None)
                or getattr(err, "status_code", None)
                or getattr(err, "http_code", None)
            )
            if isinstance(status, int):
                return status
            if hasattr(status, "value") and isinstance(status.value, int):
                return status.value
        ...
        if isinstance(ex, (APIError, ClientError, ServerError)):
            status = status_code(ex)
            if status is not None:
                return is_retryable_http_status(status)
            # If we can't parse status, fall back to message heuristics
            msg = str(ex).lower()
            if "service unavailable" in msg or "unavailable" in msg or "503" in msg:
                return True
...
def normalize_signature_bytes(signature: Any) -> bytes | None:
...
async def content(
    ...
    working_signature: bytes | None = None
    if isinstance(message, ChatMessageUser):
            ...
            for content in message.content:
                if isinstance(content, ContentReasoning):
                    content_parts.append(
                        Part(text=content.reasoning or NO_CONTENT, thought=True)
                    )
                    if content.signature:
                        try:
                            working_signature = base64.b64decode(
                                content.signature.encode()
                            )
                        except Exception:
                            logger.warning(
                    ...
                    if working_signature is not None:
                        part_to_append.thought_signature = working_signature
                        working_signature = None
                    content_parts.append(part_to_append)

        # Now handle tool calls (make sure any pending thought_signature is attached)
        if message.tool_calls is not None:
            for tool_call in message.tool_calls:
                ...
                # Prefer explicit signature stored on the ToolCall; fall back to
                # the last reasoning signature if available.
                signature = None
                if hasattr(tool_call, "thought_signature"):
                    signature = tool_call.thought_signature
                # Normalize str -> bytes if some earlier layer serialized it.
                if isinstance(signature, str):
                    sig_bytes = normalize_signature_bytes(signature)
                    signature = sig_bytes
                if signature is None:
                    signature = working_signature
        ...
        response = FunctionResponse(
            name=message.function or message.tool_call_id,
            response={
                "content": (
...
def completion_choice_from_candidate(candidate: Candidate) -> ChatCompletionChoice:
    content: list[ContentText | ContentReasoning | ContentImage | ContentAudio | ContentVideo] = []
    working_reasoning_block: ContentReasoning | None = None

    parts = candidate.content.parts if candidate.content and candidate.content.parts else None
    if parts is not None:
        for part in parts:
            thought_sig_bytes = normalize_signature_bytes(
                getattr(part, "thought_signature", None)
            )

            if getattr(part, "thought", False):
                working_reasoning_block = ContentReasoning(
                    reasoning=part.text or NO_CONTENT,
                    redacted=False,
                )
                content.append(working_reasoning_block)
                continue

                if thought_sig_bytes is not None:
                    signature_b64 = base64.b64encode(thought_sig_bytes).decode()
                    if working_reasoning_block is None:
                        ...
                        working_reasoning_block.signature = signature_b64
    ...
    if parts is not None:
        for part in parts:
            if part.function_call:
                ...
                # Store signature as base64 string to survive any serialization.
                thought_sig = (
                    base64.b64encode(thought_sig_bytes).decode()
                    if thought_sig_bytes is not None
                    else None
                )
                tool_calls.append(
    ...
    # stop reason
    stop_reason = finish_reason_to_stop_reason(candidate.finish_reason or FinishReason.STOP)

    choice_content: str | list[InspectContent] = content if len(content) > 0 else ""
...
def usage_metadata_to_model_usage(
    ...
    # Reasoning tokens (Gemini thought tokens)
    reasoning_tokens = getattr(metadata, "thoughts_token_count", None) or 0

    return ModelUsage(
        input_tokens=metadata.prompt_token_count or 0,
        output_tokens=metadata.candidates_token_count or 0,
        total_tokens=metadata.total_token_count or 0,
        input_tokens_cache_read=cached_content_token_count if cached_content_token_count > 0 else None,
