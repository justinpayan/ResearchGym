    ...
    async def search(query: str, start_idx: int) -> list[SearchLink]:
        ...
        for item in results:
            try:
                ...
                # If text is absent but highlights are present (e.g. text=False, highlights=True),
                # treat the highlights as the text payload so downstream consumers get content.
                if not text and raw_h:
                    if isinstance(raw_h, list):
                        text = " ".join(str(h) for h in raw_h)
