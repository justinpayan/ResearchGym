...
def _display_path(resolved: str, original: str, code_dir: str | None) -> str:
    """
    Return a redacted path for agent display:
    - If the user provided a relative path, keep it as-is.
    - If the resolved path is under CODE_DIR, show it relative to CODE_DIR.
    - Otherwise, fall back to the basename to avoid leaking absolute paths.
    """
    if original and not os.path.isabs(original):
        return original
    if code_dir:
        try:
            rel = Path(resolved).resolve().relative_to(Path(code_dir).resolve())
            return str(rel)
        except Exception:
            pass
...
def read_file_chunk():
    async def execute(file: str, start_line: int = 1, max_lines: int = 50) -> str:
        ...
        if max_lines > 500:
            raise ToolError("max_lines cannot exceed 200")

        # Track original input for display and a resolved path for access
        original = file
        try:
            # Resolve relative path against CODE_DIR if provided
            base_dir = os.environ.get("CODE_DIR")
            resolved_file = file
            if base_dir and not (file.startswith("/") or (len(file) > 1 and file[1] == ":")):
                resolved_file = os.path.join(base_dir, file)

            # Read the file
            content = await sandbox().read_file(resolved_file)
            ...
            display = _display_path(resolved_file, original, os.environ.get("CODE_DIR"))
            raise ToolError(f"File '{display}' not found")
...
def search_file():
    async def execute(
        ...
        try:
            ...
            display = _display_path(resolved_file, original, base_dir)

            if not all_matches:
                return f"No matches found for '{query}' in {display}"
            ...
            summary = [
                f"Found {total_matches} matches for '{query}' in {display}",
