...
class RGAgentEvolutionAdapter:
    ...
    def build_command(self, agent_root: Path, cfg: RGAgentEvolutionConfig) -> list[str]:
        # Run the vendored RGAgentEvolution start.py as a module with env vars
        python = sys.executable
        entry = agent_root / "start.py"
        cmd = [python, str(entry)]
        return cmd

    def run(self, cfg: RGAgentEvolutionConfig, agent_root: Path, dry_run: bool = True) -> Observation:
        cmd = self.build_command(agent_root, cfg)
        self.logger.info(f"RGAgentEvolution command: {' '.join(cmd)}")
        ...
        # Provide environment used by RGAgentEvolution
        env.update(
            {
                ...
                # Pass the correct log directory to RGAgentEvolution
        ...
        proc = subprocess.Popen(
            ...
            cwd=str(agent_root),
            env=env,
            stdout=open(self.env.logs_dir / "rg_agent_evolution.stdout.log", "w"),
            stderr=open(self.env.logs_dir / "rg_agent_evolution.stderr.log", "w"),
        )
